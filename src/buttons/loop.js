...
module.exports = async ({  inter, queue }) => { 
    ...
    const methods = ['disabled', 'track', 'ON'];
    ...
    if (repeatMode === 1 ) queue.setRepeatMode( QueueRepeatMode.ON)

    if (repeatMode === 2) queue.setRepeatMode( QueueRepeatMode.OFF)
    
    return inter.reply({ content: `loop made has been set to **${methods[queue.repeatMode]}**.<a:greencheck:1042472323757920336>`})
