...
module.exports = {
    ...
    async execute({ inter, client }) {
        ...
        if (!filter) return inter.reply({ content: `This filter doesn't exist ${inter.member}... try again ? <a:redcheck:1042472321329414235>\n${actualFilter ? `Filter currently active ${actualFilter}.\n` : ''}List of available filters ${filters.map(x => `**${x}**`).join(', ')}.`, ephemeral: true });
        ...
        inter.reply({ content: `The filter ${filter} is now **${queue.getFiltersEnabled().includes(filter) ? 'enabled' : 'disabled'}** <a:greencheck:1042472323757920336>\n*Reminder the longer the music is, the longer this will take.*` });
    },
};